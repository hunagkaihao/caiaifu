...
public class PlcTagValToTargetValJudge : IStep, ITransientDependency
{
    ...
    private ILogger<PlcTagValToTargetValJudge> _logger;
    ...
    public async Task<bool> Execute()
    {
        try
        {
            ...
            var targetValTag = await _variableManager.GetVariableByNameAsync(ParaVariableMaps["TargetVal"]).ConfigureAwait(false);
            if(targetValTag == null)
                throw new Exception($"不存在名为{ParaVariableMaps["TargetVal"]}的变量定义");

            string plcName = plcNameTag.Value;
            string tagName = tagNameTag.Value;
            string targetVal = targetValTag.Value;
            ...
            if(!ret)
            {
                await _monitorManager.UpdateStationStepInfoAsync(
                    RelativeStep.StationCode, 
                    $"等待Plc变量{plcName}.{tagName}变成目标值{targetVal}");
                return false;
            }

            var tagVal = _plcHelper.ReadPlcTag(plcName, tagName);
            if(tagVal == null || tagVal.Value != targetVal)
            {
                _logger.Info($"捕捉到Plc变量{plcName}.{tagName}发生变化，新值为{tagVal?.Value}，不是目标值{targetVal}");
                await _monitorManager.UpdateStationStepInfoAsync(
                    RelativeStep.StationCode, 
                    $"捕捉到Plc变量{plcName}.{tagName}发生变化，新值为{tagVal?.Value}，不是目标值{targetVal}");
                return false;
            }

            _logger.Info($"捕捉到Plc变量{plcName}.{tagName}变化成目标值{targetVal}");
            await _monitorManager.UpdateStationStepInfoAsync(
                RelativeStep.StationCode, 
                $"捕捉到Plc变量{plcName}.{tagName}变化成目标值{targetVal}");
